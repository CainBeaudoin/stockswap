...
(()=>{
  ...
  const live=document.querySelector('#portfolio .head .live');
  if(!portfolioHead||!live)return;
  const tools=document.createElement('div');
  tools.className='api-portfolio-tools';
  live.replaceWith(tools);
  tools.appendChild(live);
  const btn=document.createElement('button');
  btn.id='apiKeyButton';btn.className='api-key-btn';btn.type='button';btn.textContent='Add API Key';
  ...
  const sync=()=>{const active=!!currentKey();btn.classList.toggle('active',active);btn.textContent=active?'API Key Connected':'Add API Key';status.classList.toggle('active',active);status.querySelector('span').textContent=active?'Browser API key active':'Using server configuration'};
